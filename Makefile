...
vendor:
	...
	curl -sSL https://github.com/tmon546596046/justfortest/releases/download/v0.0.1/bb.tar.gz -o bb.tar.gz
	tar -zxf bb.tar.gz -C src/git/inspursoft
	export GOPATH=`pwd`; cd src/git/inspursoft/board/src/apiserver; ../../../dep ensure -v; 
	docker build --pull -t piranhahu/vendor:1.0 .
	docker login -u $(GITHUB_USERNAME) -p $(GITHUB_PASSWORD)
