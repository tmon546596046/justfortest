...
kube::version::get_version_vars() {
  ...
  if [[ '%' == "%" ]]; then
    KUBE_GIT_COMMIT='a87e9a978f65a8303aa9467537aa59c18122cbf9'
    KUBE_GIT_TREE_STATE="archive"
    # When a 'git archive' is exported, the 'tag: v1.14.4, refs/pull/79831/head' below will look
    # something like 'HEAD -> release-1.8, tag: v1.8.3' where then 'tag: '
    # can be extracted from it.
    if [[ 'tag: v1.14.4, refs/pull/79831/head' =~ tag:\ (v[^ ,]+) ]]; then
